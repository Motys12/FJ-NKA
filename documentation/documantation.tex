...
\begin{enumerate}
    \item \texttt{ab}: Neakceptované
    \item \texttt{c}: Neakceptované
    \item \texttt{a}: Neakceptované
    \item \texttt{abcdef}: Akceptované
    \item \texttt{} (prázdny reťazec): Neakceptované
\end{enumerate}

\subsection{Problémy a ich riešenie}
Počas implementácie som sa stretol s problémom, že pri použití zátvoriek (\{\}), ktoré označujú opakovanie, sa môj vstup neopakoval a slovo nebolo akceptované správne. 

Aby som tento problém vyriešil, vytvoril som boolean premennú has\_LCBRA, ktorá označuje, že v aktuálnom výraze sa nachádza ľavá zátvorka. Táto premenná umožňuje rozpoznať, kedy sa nachádzame vnútri zátvoriek, a preto sme po jej identifikovaní vygenerovali epsilonový prechod, ktorý umožnil "preskočiť" späť na štartovací stav. Týmto spôsobom sa zabezpečilo, že výraz s opakovaním fungoval správne a akceptoval slová, ktoré by sa inak nezrealizovali.

\subsection{Hodnotenie zadania}

V celkovom kontexte bolo zadanie veľmi zaujímavé, pretože si vyžiadalo podrobné porozumenie konceptu práce NKA. Aby sme správne implementovali generátor NKA na základe regulárnych výrazov, bolo potrebné dôkladne pochopiť, ako NKA funguje a ako môžeme využiť jeho vlastnosti na správne spracovanie rôznych komponentov regulárnych výrazov, ako sú sekvencie, alternatívy a opakovania.

\section{Vyhodnotenie a záver}

Počas implementácie sa mi podarilo správne vybudovať štruktúru stromu a implementovať základné komponenty regulérnych výrazov, ako sú sekvencie, alternatívy a opakovanie symbolov. 

Okrem toho som implementoval správnu logiku pre spracovanie epsilonových prechodov v prípadoch s opakovaním, čo umožnilo správnu interpretáciu regulérnych výrazov s opakujúcimi sa vzormi.

Aj keď úloha bola náročná, považujem ju za veľmi poučnú, pretože poskytla príležitosť na hlbšie pochopenie koncepcie regulérnych výrazov a ich implementácie v podobe NKA.
